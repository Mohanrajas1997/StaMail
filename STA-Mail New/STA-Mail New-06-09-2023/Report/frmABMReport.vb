...
#End Region
    ...
    Private Sub LoadData()
        ...
        If txtmobileno.Text <> "" Then
            lsCondn &= " and a.abm_MobileNo = '" & QuoteFilter(txtmobileno.Text.Trim) & "' "
        End If

        If txtClientName.Text.Trim <> "" Then
            lsCondn &= " and a.abm_ClientName = '" & QuoteFilter(txtClientName.Text.Trim) & "'"
        End If

        If txtbranchId.Text.Trim <> "" Then
            lsCondn &= " and a.abm_branchID = '" & QuoteFilter(txtbranchId.Text.Trim) & "'"
        End If

        If cboFileName.Text.Trim <> "" Then
            lsCondn &= " and a.branchfile_gid = '" & cboFileName.SelectedValue.ToString & "'"
        End If

        If txtAmount.Text.Trim <> "" Then
            lsCondn &= " and a.abm_DebitAmount = '" & txtAmount.Text.Trim & "'"
        End If

        If cboDepostion.SelectedValue <> "" Then
            lsCondn &= " and a.abm_Dispostion1 = '" & cboDepostion.SelectedValue & "'"
        End If
        ...
        fsSql &= " b.import_by as 'Import By',a.sendmail_flag as 'Send Mail'"
    ...
    Private Sub frmAllocationReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        fsSql = ""
        fsSql = "SELECT  abm_Dispostion1 FROM abml.abm_trn_tbranch  group by abm_Dispostion1"

        gpBindCombo(fsSql, "abm_Dispostion1", "abm_Dispostion1", cboDepostion, gOdbcConn)
    ...
    Private Sub btnexport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnexport.Click
        If dGrid.RowCount > 0 Then
            PrintDGViewXML(dGrid, gsReportPath & "ABMLReport.xls", "ABML Report")
        Else
    ...
    Private Sub dtpImportFrom_ValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dtpImportFrom.ValueChanged
        ...
        fsSql = ""
        fsSql &= " select file_name,branchfile_gid from  abm_trn_tbranchfile where 1=1"
    ...
    Private Sub cboFileName_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboFileName.TextChanged, cboDepostion.TextChanged
        If cboFileName.SelectedIndex = -1 And cboFileName.Text <> "" Then gpAutoFillCombo(cboFileName)
    End Sub
    Private Sub cboDepostion_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboFileName.TextChanged, cboDepostion.TextChanged
        If cboFileName.SelectedIndex = -1 And cboFileName.Text <> "" Then gpAutoFillCombo(cboFileName)
    End Sub
End Class